...
def get_schoolfund_debits(start_date, end_date):
    debits = []
    ...
    # Fetch Payment Vouchers within the specified period for 'schoolfund'
    payment_vouchers = RMIPaymentVoucher.objects.filter(date__gte=start_date, date__lt=end_date, vote_head='school_fund')
...
def get_schoolfund_credits(start_date, end_date):
    credits = []
    ...
    # Fetch RMI Receipts within the specified period where received_from is 'schoolfund'
    schoolfund_receipts = RMIReceipt.objects.filter(
        received_from='schoolfund',
        date__gte=start_date,
        date__lt=end_date
    )

    # Loop through the filtered receipts and create credit entries (date, amount, and cashbook)
    for receipt in schoolfund_receipts:
        credits.append({
            "date": receipt.date,
...
def get_schoolfund_ledger(start_date, end_date):
    # Get debits and credits
    debits = get_schoolfund_debits(start_date, end_date)
    credits = get_schoolfund_credits(start_date, end_date)
