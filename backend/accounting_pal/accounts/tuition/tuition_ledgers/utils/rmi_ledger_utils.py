...
def get_rmi_debits(start_date, end_date):
    ...
    payment_vouchers = TuitionPaymentVoucher.objects.filter(date__gte=start_date, date__lt=end_date, vote_head='rmi')
...
def get_rmi_credits(start_date, end_date):
    ...
    operation_receipts = TuitionReceipt.objects.filter(date__gte=start_date, date__lt=end_date, received_from='rmi')
