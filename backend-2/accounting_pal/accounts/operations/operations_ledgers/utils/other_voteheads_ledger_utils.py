...
def get_other_voteheads_debits(start_date, end_date):
    ...
    payment_vouchers = PaymentVoucher.objects.filter(date__gte=start_date, date__lt=end_date, vote_head='other_voteheads')
...
def get_other_voteheads_credits(start_date, end_date):
    ...
    operation_receipts = OperationReceipt.objects.filter(
            date__gte=start_date, 
            date__lt=end_date, 
            other_voteheads__gt=0
        )
