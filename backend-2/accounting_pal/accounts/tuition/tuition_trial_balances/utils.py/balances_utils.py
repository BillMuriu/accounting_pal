...
from accounts.tuition.tuition_bankcharges.models import TuitionBankCharge  # Use Tuition Bank Charges
from accounts.tuition.tuition_paymentvouchers.models import TuitionPaymentVoucher  # Use Tuition Payment Vouchers
from accounts.tuition.tuition_pettycash.models import TuitionPettyCash  # Use Tuition Petty Cash
from accounts.tuition.tuition_receipts.models import TuitionReceipt  # Use Tuition Receipts
from accounts.tuition.tuition_balances.models import TuitionOpeningBalance  # Use Tuition Opening Balance


def calculate_tuition_balances(start_date, end_date):
    ...
    opening_balance_record = TuitionOpeningBalance.objects.filter(date__lt=start_date).order_by('-date').first()
    ...
    total_bank_receipts_start = Decimal(TuitionReceipt.objects.filter(
        cash_bank='bank',
        date__lt=start_date
    ).aggregate(total=Sum('total_amount'))['total'] or 0)

    # Total cash receipts up to the start date
    total_cash_receipts_start = Decimal(TuitionReceipt.objects.filter(
        cash_bank='cash',
        date__lt=start_date
    ).aggregate(total=Sum('total_amount'))['total'] or 0)

    # Total bank charges up to the start date
    total_bank_charges_start = Decimal(TuitionBankCharge.objects.filter(
        charge_date__lt=start_date
    ).aggregate(total=Sum('amount'))['total'] or 0)

    # Total petty cash up to the start date
    total_petty_cash_start = Decimal(TuitionPettyCash.objects.filter(
        date_issued__lt=start_date
    ).aggregate(total=Sum('amount'))['total'] or 0)

    # Total bank payment vouchers up to the start date
    total_bank_payment_vouchers_start = Decimal(TuitionPaymentVoucher.objects.filter(
        payment_mode='bank',
        date__lt=start_date
    ).aggregate(total=Sum('amount_shs'))['total'] or 0)

    # Total cash payment vouchers up to the start date
    total_cash_payment_vouchers_start = Decimal(TuitionPaymentVoucher.objects.filter(
        payment_mode='cash',
        date__lt=start_date
    ).aggregate(total=Sum('amount_shs'))['total'] or 0)
    ...
    closing_balance_record = TuitionOpeningBalance.objects.filter(date__lte=end_date).order_by('-date').first()
    ...
    total_bank_receipts_end = Decimal(TuitionReceipt.objects.filter(
        cash_bank='bank',
        date__lte=end_date
    ).aggregate(total=Sum('total_amount'))['total'] or 0)

    # Total cash receipts up to the end date
    total_cash_receipts_end = Decimal(TuitionReceipt.objects.filter(
        cash_bank='cash',
        date__lte=end_date
    ).aggregate(total=Sum('total_amount'))['total'] or 0)

    # Total bank charges up to the end date
    total_bank_charges_end = Decimal(TuitionBankCharge.objects.filter(
        charge_date__lte=end_date
    ).aggregate(total=Sum('amount'))['total'] or 0)

    # Total petty cash up to the end date
    total_petty_cash_end = Decimal(TuitionPettyCash.objects.filter(
        date_issued__lte=end_date
    ).aggregate(total=Sum('amount'))['total'] or 0)

    # Total bank payment vouchers up to the end date
    total_bank_payment_vouchers_end = Decimal(TuitionPaymentVoucher.objects.filter(
        payment_mode='bank',
        date__lte=end_date
    ).aggregate(total=Sum('amount_shs'))['total'] or 0)

    # Total cash payment vouchers up to the end date
    total_cash_payment_vouchers_end = Decimal(TuitionPaymentVoucher.objects.filter(
        payment_mode='cash',
        date__lte=end_date
    ).aggregate(total=Sum('amount_shs'))['total'] or 0)
